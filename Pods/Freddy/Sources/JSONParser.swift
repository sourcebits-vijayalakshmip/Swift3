...
public struct JSONParser {
    ...
    fileprivate let input: UnsafeBufferPointer<UInt8>
    fileprivate var loc = 0
    ...
    fileprivate mutating func parseValue() throws -> JSON {
    ...
    fileprivate mutating func guardAgainstUnsupportedEncodings() throws {
    ...
    fileprivate mutating func decodeNull() throws -> JSON {
    ...
    fileprivate mutating func decodeTrue() throws -> JSON {
        guard input.index(loc, offsetBy: 3, limitedBy: input.count) != input.count else {
            throw Error.literalNilMisspelled(offset: loc)
        }
    ...
    fileprivate mutating func decodeFalse() throws -> JSON {
    ...
    fileprivate var stringDecodingBuffer = [UInt8]()
    fileprivate mutating func decodeString() throws -> JSON {
        loc = (loc + 1)
    ...
    fileprivate mutating func readCodeUnit() -> UInt16? {
    ...
    fileprivate mutating func readUnicodeEscape(_ start: Int) throws {
    ...
    fileprivate mutating func decodeArray() throws -> JSON {
        let start = loc
        loc = (loc + 1)
        var items = [JSON]()
    ...
    fileprivate struct DecodeObjectBuffers {
    ...
    fileprivate var decodeObjectBuffers = DecodeObjectBuffers()

    fileprivate mutating func decodeObject() throws -> JSON {
        let start = loc
        loc = (loc + 1)
    ...
    fileprivate mutating func decodeIntegralValue(_ parser: NumberParser) throws -> JSON {
    ...
    fileprivate mutating func decodeFloatingPointValue(_ parser: NumberParser, sign: Sign, value: Double) throws -> JSON {
        var parser = parser
        var value = value
    ...
    fileprivate mutating func decodeNumberAsString(from position: Int) throws -> JSON {
    ...
    fileprivate func detectingFloatingPointErrors<T>(start loc: Int, _ f: () throws -> T) throws -> T {
