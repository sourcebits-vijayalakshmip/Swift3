...
public struct JSONEncodingDetector {
    ...
    fileprivate typealias EncodingBytePrefix = (UInt8, UInt8, UInt8?, UInt8?)

    fileprivate static func prefixFromHeader(_ header: RandomAccessSlice<UnsafeBufferPointer<UInt8>>) -> EncodingBytePrefix? {
        if header.count >= 4 {
    ...
    fileprivate static func encodingFromBOM(_ prefix: EncodingBytePrefix) -> ByteStreamPrefixInformation? {
        switch prefix {
        case(0xFE, 0xFF, _, _):
            return (.utf16BE, 2)
